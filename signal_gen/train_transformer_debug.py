...
for batch_idx, (X,gen,y) in enumerate(train_loader):
    ...
    y = y[0:1,:,:]
    gen = gen[0:1,:,:]
    X = X.repeat(BATCH_SIZE,1,1).to(DEVICE, non_blocking=True)
    y = y.repeat(BATCH_SIZE,1,1).to(DEVICE, non_blocking=True)
    gen = gen.repeat(BATCH_SIZE,1,1).to(DEVICE, non_blocking=True)
    break

for batch_idx in range(NUM_EPOCH):
    X = X.detach()
    y = y.detach()
    gen = gen.detach()
