...
block_types = ['fourier', 'generic']
#block_types = ['generic']
#block_types = ['generic', 'generic']
#block_types = ['fourier']#, 'fourier']#, 'fourier', 'fourier']
...
# Setup data loaders
train = PeriodicTrain_Mask(min_time, num_freqs, avg_freq_amp, 
...
model = NBeats_Model(device=DEVICE, input_size=num_samples, 
    ...
    min_freq=lowpass, max_freq=highpass, time_len=max_time)
...
for batch_idx, (X,mult,y) in enumerate(train_loader):
    ...
    if batch_idx == NUM_EPOCH - 1:
        ...
        #print(y_p[0,0,:])
        #print(y[0,0,:])
        #show = torch.cat((y[0:1,:],y_p[0:1,:]),0)
        ...
        plot_loss(model, skip_beg=batch_idx // 2, 
            legend=['Mask Sequence Prediction'])
...
plot_loss(model, legend=['Mask Sequence Prediction'])
plt.savefig(curr_path + '/results/' + name + '_loss.png')
plt.clf()
plot_loss(model, legend=['Mask Sequence Prediction'], skip_beg=(NUM_EPOCH * 2)// 3)
plt.savefig(curr_path + '/results/' + name + '_loss2.png')
plt.clf()
plot_seq(y, y_p)
plt.savefig(seq_plot_path)
